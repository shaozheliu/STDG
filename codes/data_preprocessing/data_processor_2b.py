...
class DataProcess():
    ...
    def __init__(self,
        ...
        if choose2aor2b == 1:
            ...
            data, label, domain_label = self.import_subjecti_data_2b(False, subject_list, domain_flg)
        ...
        # self.domain_label = domain_label[index_k]
        self.data = data
        self.label = label
    ...
    def load_npy_for_2b(self, data_files, data_path):
        ...
        for i in range(len(data_files)):
            ...
            if i == 0:
                ...
                label = [one_label.tolist().index(1) for one_label in label]  # 找到下标是1的位置
            else:
                data_t = np.load(data_file)
                label_t = np.load(label_file)
                label_t = [one_label.tolist().index(1) for one_label in label_t]  # 找到下标是1的位置
    ...
    def import_subjecti_data_2b(self, every, subject_list, domain_flg):
        domainset = list(set(i[0:3] for i in subject_list))
        data = []
        label = []
        domain_label = []
        for i in range(len(subject_list)):
            domain_value = domainset.index(subject_list[i][0:3])
            data_file = self.data_path + str(subject_list[i]) + "_data_raw.npy"
            label_file = self.data_path + str(subject_list[i]) + "_label.npy"
            if i == 0:
                data = np.load(data_file)
                label = np.load(label_file)
                label = np.array([one_label.tolist().index(1) for one_label in label])  # 找到下标是1的位置
                if domain_flg == True:
                    domain_label = np.ones(data.shape[0], dtype=int) * domain_value  # 这个dtype可能有问题
            else:
                data_t = np.load(data_file)
                label_t = np.load(label_file)
                label_t = np.array([one_label.tolist().index(1) for one_label in label_t])  # 找到下标是1的位置
                if domain_flg == True:
                    # domain_label_t = np.ones(data_t.shape[0], dtype=int) * int(subject_list[i][2])  # 这个dtype可能有问题
...
if __name__ == '__main__':
    ...
    choose2aor2b = 2
    GetData = DataProcess(data_2b_path, data_2b_files, choose2aor2b,["B0101T","B0102T"])
    data = GetData.data
    label = GetData.label